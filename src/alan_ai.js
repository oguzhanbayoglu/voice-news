...
intent("Give me the news from $(source* (.*))", (p) => {
  let NEWS_API_URL = `${process.env.REACT_APP_NEWS_API_URL}`;
  ...
  api.request(
    ...
    (error, response, body) => {
      ...
      p.play(`Here are (latest|recent) news from ${p.source.value}`);
...
intent(
  ...
  (p) => {
    let NEWS_API_URL = `https://newsapi.org/v2/top-headlines?country=us`;
